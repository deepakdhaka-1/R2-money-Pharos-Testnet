...
for idx, raw_key in enumerate(keys, 1):
    ...
    if not m:
        print(f"❌ Skipping line {idx}: not a valid private key")
        continue
    ...
    # Build calldata for this wallet
    ...
    try:
        ...
        print(f"✅ Submitted TX: {w3.to_hex(tx_hash)}")
    except Exception as e:
        print(f"❌ Failed to send TX for {WALLET}: {e}")
